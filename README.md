...
joined by a thread of light. Ask a question and Dio answers **only** from
...
```bash
git clone https://github.com/Avyansh-AI/Dio-.git
cd Dio-
...
2. **Add your API key.** Open `config.json` in the project root and paste in
   ...
   Dio tells you so and suggests live ids it can actually see.
   ...
   **No API key?** Leave it as `PUT-YOUR-KEY-HERE` and Dio falls back to the
...
| `python3 server.py` | Serves the viewer on **port 4700** and runs the brain. `DIO_PORT=8080 python3 server.py` to change the port. |
...
- **Type a question** in the bar at the bottom. Dio answers in two or three
...
- **Say "remember that…"** and Dio writes a real markdown note into
...
questions about Dio?"*, and something your notes **don't** cover, to hear it
admit it.
...
notes/                   your notes (Upgrading Dio.md is the seed)
