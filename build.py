...
STOPWORDS = {
    ...
    "dio", "please", "show", "give", "find", "list",
}
...
def write_graph_data(graph: dict, out_path: str) -> None:
    ...
    with open(out_path, "w", encoding="utf-8") as fh:
        fh.write("// DIO knowledge galaxy - generated by build.py, do not edit.\n")
        fh.write(
    ...
    print(
        "DIO: indexed %d note(s) and %d link(s) -> %s"
        % (len(graph["nodes"]), len(graph["links"]), out_path)
    )


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Build the DIO knowledge graph.")
